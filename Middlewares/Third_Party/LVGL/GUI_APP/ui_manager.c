...
SensorType_t ui_get_active_sensor(void) {
    return g_active_sensor_for_details;
}
...
void ui_load_screen(ui_screen_t screen) {
    ...
    switch(screen) {
        ...
        case UI_SCREEN_DASHBOARD:
            ...
            break;
        case UI_SCREEN_SENSORS_DETAILS:  // [CHANGED] 新枚举名
            ui_screen_sensors_details_init(g_current_screen_container);  // [CHANGED] 新函数名
            break;
        case UI_SCREEN_SENSORS_LISTS:  // [CHANGED] 新枚举名
            ui_screen_sensors_lists_init(g_current_screen_container);  // [CHANGED] 新函数名
            break;
        case UI_SCREEN_DEVICE_DETAILS:
            ui_screen_devices_details_init(g_current_screen_container, g_active_device_type);
            break;
