...
static void create_single_data_panel(lv_obj_t* parent, int grid_col, const char* title, const char* unit, lv_obj_t** value_label, SensorType_t sensor_type);
...
static void sync_led_controls_from_driver(void)
{
    ...
    if (mode == LED_MODE_MANUAL) {
        ...
        switch (state) {
            case LED_STATE_SLOT_1:
                if (Drivers_RGBLED_GetSlotColor(1, &slot_color)) {
                    lv_led_set_color(g_ui.led_indicator, lv_color_make(slot_color.R, slot_color.G, slot_color.B));
        ...
        if (current_color.R == 0 && current_color.G == 0 && current_color.B == 0) {
            ...
            lv_led_set_color(g_ui.led_indicator, lv_color_make(current_color.R, current_color.G, current_color.B));
            lv_led_on(g_ui.led_indicator);
        }
    }
}
...
static void sensor_data_update_cb(lv_timer_t *timer)
{
    ...
    if (SensorTask_GetSensorData(SENSOR_TYPE_SHT30, &data) && data.is_valid) {
        if (g_ui.temp_label) lv_label_set_text_fmt(g_ui.temp_label, "%.1f", data.values.sht30.temp);
        if (g_ui.humi_label) lv_label_set_text_fmt(g_ui.humi_label, "%.1f", data.values.sht30.humi);
    } else {
        if (g_ui.temp_label) lv_label_set_text(g_ui.temp_label, "--.-");
        if (g_ui.humi_label) lv_label_set_text(g_ui.humi_label, "--.-");
    }

    /* 光照 */
    if (SensorTask_GetSensorData(SENSOR_TYPE_GY30, &data) && data.is_valid) {
        if (g_ui.light_label) lv_label_set_text_fmt(g_ui.light_label, "%d", (int)data.values.gy30.lux);
        ...
        if (g_ui.light_label) lv_label_set_text(g_ui.light_label, "--");
    }

    /* 烟感 */
    if (SensorTask_GetSensorData(SENSOR_TYPE_SMOKE, &data) && data.is_valid) {
        if (g_ui.smoke_label) lv_label_set_text_fmt(g_ui.smoke_label, "%d", data.values.smoke.ppm);
    } else {
        if (g_ui.smoke_label) lv_label_set_text(g_ui.smoke_label, "--");
    }
}
...
static void data_panel_click_event_cb(lv_event_t *e)
{
    SensorType_t target_sensor = (SensorType_t)(intptr_t)lv_event_get_user_data(e);
...
static void led_mode_btn_event_cb(lv_event_t *e)
{
    ...
    if (current_mode == LED_MODE_MANUAL) {
        ...
        lv_label_set_text(lv_obj_get_child(g_ui.led_mode_btn, 0), LV_SYMBOL_REFRESH " 自动");
        lv_obj_add_state(g_ui.led_cycle_btn, LV_STATE_DISABLED);
    } else {
        Drivers_RGBLED_SetMode(LED_MODE_MANUAL);
        lv_label_set_text(lv_obj_get_child(g_ui.led_mode_btn, 0), LV_SYMBOL_SETTINGS " 手动");
        lv_obj_clear_state(g_ui.led_cycle_btn, LV_STATE_DISABLED);
    }
}
...
static void beep_switch_event_cb(lv_event_t *e)
{
    ...
    if (lv_obj_has_state(sw, LV_STATE_CHECKED)) {
        ...
        lv_anim_start(&g_ui.beep_anim);
        if (g_ui.beep_once_timer) lv_timer_del(g_ui.beep_once_timer);
...
static void create_temp_humi_panel(lv_obj_t* parent, int grid_col)
{
    ...
    lv_obj_set_grid_cell(panel, LV_GRID_ALIGN_STRETCH, grid_col, 2, LV_GRID_ALIGN_STRETCH, 0, 1);
    lv_obj_add_flag(panel, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(panel, data_panel_click_event_cb, LV_EVENT_CLICKED, (void*)SENSOR_TYPE_SHT30);
    ...
    lv_obj_set_flex_align(data_container, LV_FLEX_ALIGN_SPACE_AROUND, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    ...
    lv_obj_set_flex_align(humi_container, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
...
{
    lv_obj_t * panel = lv_obj_create(parent);
    lv_obj_set_grid_cell(panel, LV_GRID_ALIGN_STRETCH, grid_col, 1, LV_GRID_ALIGN_STRETCH, 0, 1);
    lv_obj_add_flag(panel, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(panel, data_panel_click_event_cb, LV_EVENT_CLICKED, (void*)sensor_type);
    ...
    lv_obj_set_flex_align(value_container, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
...
static void create_led_panel(lv_obj_t* parent, int grid_col, int grid_row)
{
    ...
    lv_obj_set_grid_cell(g_ui.led_panel, LV_GRID_ALIGN_STRETCH, grid_col, 1, LV_GRID_ALIGN_STRETCH, grid_row, 1);
    lv_obj_set_style_pad_all(g_ui.led_panel, 15, 0);
    lv_obj_add_flag(g_ui.led_panel, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(g_ui.led_panel, led_panel_click_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_set_style_border_opa(g_ui.led_indicator, LV_OPA_COVER, 0);  // 完全不透明
    ...
    lv_obj_add_event_cb(g_ui.led_cycle_btn, led_cycle_btn_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(g_ui.led_mode_btn, led_mode_btn_event_cb, LV_EVENT_CLICKED, NULL);
}
...
{
    lv_obj_t * panel = lv_obj_create(parent);
    lv_obj_set_grid_cell(panel, LV_GRID_ALIGN_STRETCH, grid_col, 1, LV_GRID_ALIGN_STRETCH, grid_row, 1);

    g_ui.beep_img = lv_img_create(panel);
    lv_img_set_src(g_ui.beep_img, &beep_symbol);
    lv_obj_align(g_ui.beep_img, LV_ALIGN_CENTER, 0, -10);
    lv_img_set_pivot(g_ui.beep_img, lv_obj_get_width(g_ui.beep_img)/2, lv_obj_get_height(g_ui.beep_img)/2);
...
static void create_gif_panel(lv_obj_t* parent, int grid_col, int grid_row)
{
    ...
    lv_obj_add_event_cb(other_switch, gif_switch_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
}
...
void ui_screen_dashboard_init(lv_obj_t* parent)
{
    ...
    static lv_coord_t data_col[] = {LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
    ...
    create_single_data_panel(data_grid, 2, "光照", "Lux", &g_ui.light_label, SENSOR_TYPE_GY30);
    create_single_data_panel(data_grid, 3, "烟感", "PPM", &g_ui.smoke_label, SENSOR_TYPE_SMOKE);
    ...
    static lv_coord_t ctrl_col[] = {LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
