 ...
 * @brief   LVGL 登录屏幕模块
 * @details
 * 实现登录界面的UI创建和事件处理逻辑，包括模态覆盖层、密码显示/隐藏、键盘交互等。
 * @author  MmsY
 * @time    2025/11/23
 ******************************************************************************
 */
 ...
 * @brief 统一的消息框关闭逻辑。
 * @details
 * 隐藏消息框、其覆盖层，并确保键盘及覆盖层也被隐藏，同时清除输入框焦点。
 * 这是处理UI状态重置的核心函数。
 */
static void hide_msgbox_and_overlay(void) {
  if (g_ui.msgbox)
    lv_obj_add_flag(g_ui.msgbox, LV_OBJ_FLAG_HIDDEN);
  if (g_ui.msgbox_overlay)
    lv_obj_add_flag(g_ui.msgbox_overlay, LV_OBJ_FLAG_HIDDEN);

  // 清空输入框
  if (g_ui.user_name_input)
    lv_textarea_set_text(g_ui.user_name_input, "");
  if (g_ui.password_input)
    lv_textarea_set_text(g_ui.password_input, "");
 ...
 */
static void dismiss_all_keyboards(void) {
  if (g_ui.user_name_keyboard)
    lv_obj_add_flag(g_ui.user_name_keyboard, LV_OBJ_FLAG_HIDDEN);
  if (g_ui.password_keyboard)
    lv_obj_add_flag(g_ui.password_keyboard, LV_OBJ_FLAG_HIDDEN);
  if (g_ui.keyboard_overlay)
    lv_obj_add_flag(g_ui.keyboard_overlay, LV_OBJ_FLAG_HIDDEN);
  if (g_ui.user_name_input)
    lv_obj_clear_state(g_ui.user_name_input, LV_STATE_FOCUSED);
  if (g_ui.password_input)
    lv_obj_clear_state(g_ui.password_input, LV_STATE_FOCUSED);
}
...
static void login_btn_click_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    // 验证用户名和密码
    if ((strcmp(user_txt, "stm32") == 0) &&
        (strcmp(password_txt, "123456") == 0)) {
      ui_load_screen(UI_SCREEN_DASHBOARD);
    } else {
      // 显示错误提示
      if (g_ui.msgbox_overlay)
        lv_obj_clear_flag(g_ui.msgbox_overlay, LV_OBJ_FLAG_HIDDEN);
      if (g_ui.msgbox) {
...
static void password_eye_click_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    lv_label_set_text(g_ui.password_eye_icon, is_password_mode
                                                  ? LV_SYMBOL_EYE_CLOSE
                                                  : LV_SYMBOL_EYE_OPEN);
  }
}
...
static void create_message_box(lv_obj_t *parent) {
  ...
  lv_obj_add_flag(g_ui.msgbox_overlay, LV_OBJ_FLAG_HIDDEN);
  lv_obj_add_event_cb(g_ui.msgbox_overlay, msgbox_close_event_cb,
                      LV_EVENT_CLICKED, NULL);

  // 2. 创建消息框本体
  g_ui.msgbox = lv_msgbox_create(parent, "登录失败", "账号或密码错误, 请重试!",
                                 NULL, true);
  ...
  if (close_btn) {
    lv_obj_add_event_cb(close_btn, msgbox_close_event_cb,
                        LV_EVENT_CLICKED | LV_EVENT_PREPROCESS, NULL);
  }
}
...
static void create_login_widgets(lv_obj_t *parent) {
  ...
  lv_obj_set_flex_align(user_line, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
  ...
  lv_obj_set_style_text_color(user_icon, lv_palette_main(LV_PALETTE_BLUE_GREY),
                              0);
  ...
  lv_obj_add_event_cb(g_ui.user_name_input, textarea_focus_event_cb,
  ...
  lv_obj_set_flex_align(pass_line, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  ...
  lv_obj_set_style_text_color(pass_icon, lv_palette_main(LV_PALETTE_BLUE_GREY),
                              0);
  ...
  lv_obj_add_event_cb(g_ui.password_input, password_focus_event_cb,
                      LV_EVENT_FOCUSED, NULL);
  ...
  lv_obj_set_style_text_color(g_ui.password_eye_icon,
                              lv_palette_main(LV_PALETTE_GREY), 0);
  lv_obj_add_flag(g_ui.password_eye_icon, LV_OBJ_FLAG_CLICKABLE);
  lv_obj_add_event_cb(g_ui.password_eye_icon, password_eye_click_event_cb,
                      LV_EVENT_CLICKED, NULL);
  ...
  lv_obj_add_event_cb(login_btn, login_btn_click_event_cb, LV_EVENT_CLICKED,
                      NULL);
  ...
  lv_obj_add_flag(g_ui.user_name_keyboard, LV_OBJ_FLAG_HIDDEN);
  lv_obj_add_event_cb(g_ui.user_name_keyboard, keyboard_event_cb, LV_EVENT_ALL,
                      NULL);

  g_ui.password_keyboard = lv_keyboard_create(parent);
  lv_keyboard_set_mode(g_ui.password_keyboard, LV_KEYBOARD_MODE_NUMBER);
  lv_obj_add_flag(g_ui.password_keyboard, LV_OBJ_FLAG_HIDDEN);
  lv_obj_add_event_cb(g_ui.password_keyboard, keyboard_event_cb, LV_EVENT_ALL,
                      NULL);
  ...
  lv_obj_add_flag(g_ui.keyboard_overlay, LV_OBJ_FLAG_HIDDEN);
  lv_obj_add_event_cb(g_ui.keyboard_overlay, keyboard_overlay_click_event_cb,
                      LV_EVENT_CLICKED, NULL);
}
