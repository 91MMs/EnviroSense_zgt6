...
bool SensorTask_Init(void) {
    if (g_sensor_manager.is_initialized) return true;
    ...
    g_sensor_manager.global_data_mutex = osMutexCreate(osMutex(GlobalSensorMutex));
    ...
    for (int i = 0; i < SENSOR_TYPE_MAX; i++) {
        ...
        snprintf(g_sensor_manager.sensors[i].name, SENSOR_MAX_NAME_LEN, "Sensor_%d", i);
    }
    
    // 创建传感器任务
    osThreadDef(sensorTask, SensorTask_MainLoop, SENSOR_TASK_PRIORITY, 0, SENSOR_TASK_STACK_SIZE);
...
bool SensorTask_RegisterSensor(SensorType_t type, 
    ...
    if (!g_sensor_manager.is_initialized || type >= SENSOR_TYPE_MAX || type == SENSOR_TYPE_NONE || callbacks == NULL || callbacks->init_func == NULL || callbacks->read_func == NULL) {
...
bool SensorTask_EnableSensor(SensorType_t type) {
    if (!g_sensor_manager.is_initialized || type >= SENSOR_TYPE_MAX || type == SENSOR_TYPE_NONE) {
        return false;
    }
    ...
    if (!sensor->is_enabled) {
        ...
        // 通知状态变化事件
        SensorTask_NotifyEvent(SENSOR_EVENT_STATUS_CHANGE, type, NULL, SENSOR_STATUS_INITIALIZING);
    }
    return true;
}
...
bool SensorTask_DisableSensor(SensorType_t type) {
    ...
    if (sensor->is_enabled) {
        ...
        // 通知状态变化事件
        SensorTask_NotifyEvent(SENSOR_EVENT_STATUS_CHANGE, type, NULL, SENSOR_STATUS_OFFLINE);
    }
...
bool SensorTask_GetSensorStatus(SensorType_t type, SensorStatus_t* status) {
    if (!g_sensor_manager.is_initialized || type >= SENSOR_TYPE_MAX || status == NULL) {
        return false;
    }
        ...
        for (int i = 1; i < SENSOR_TYPE_MAX; i++) {  // 跳过SENSOR_TYPE_NONE
            ...
            if (sensor->status == SENSOR_STATUS_INITIALIZING) {
                if (SensorTask_InitializeSensor(sensor)) {
                    ...
                    // 通知状态变化事件
                    SensorTask_NotifyEvent(SENSOR_EVENT_STATUS_CHANGE, (SensorType_t)i, NULL, SENSOR_STATUS_ONLINE);
                } else {
            ...
            if ((current_time - sensor->last_update_time) >= sensor->update_interval_ms) {
                if (SensorTask_UpdateSensor(sensor)) {
                    sensor->last_update_time = current_time;
                    
                    // 通知数据更新事件
                    SensorTask_NotifyEvent(SENSOR_EVENT_DATA_UPDATE, (SensorType_t)i, &sensor->data, sensor->status);
                } else {
                    SensorTask_HandleSensorError(sensor);
                }
            }
...
static bool SensorTask_UpdateSensor(SensorInstance_t* sensor) {
    ...
    if (callbacks->read_func != NULL) {
        ...
        if (result) {
            if (osMutexWait(g_sensor_manager.global_data_mutex, osWaitForever) == osOK) {
                ...
                if (sensor->history_count == 1) { // 如果是第一个数据点
                    ...
                    // 更新最大/最小值
                    if (primary_value < sensor->stats.min) sensor->stats.min = primary_value;
                    if (primary_value > sensor->stats.max) sensor->stats.max = primary_value;
                    if (sensor->type == SENSOR_TYPE_SHT30) {
                        if (secondary_value < sensor->secondary_stats.min) sensor->secondary_stats.min = secondary_value;
                        if (secondary_value > sensor->secondary_stats.max) sensor->secondary_stats.max = secondary_value;
                    }
                    
                    // 重新计算平均值 (对于少量数据，直接重算最准确)
                    int start_idx = (sensor->history_head - sensor->history_count + SENSOR_HISTORY_SIZE) % SENSOR_HISTORY_SIZE;
                    float p_sum = 0.0f, p_min = sensor->history[start_idx], p_max = sensor->history[start_idx];
                    ...
                    for (int i = 0; i < sensor->history_count; i++) {
                        ...
                        if (sensor->type == SENSOR_TYPE_SHT30) {
                            ...
                            s_sum += s_val;
                            if (s_val < s_min) s_min = s_val;
                            if (s_val > s_max) s_max = s_val;
                        }
                    }
                    ...
                    sensor->stats.avg = sensor->stats.local_avg; // 全局平均值也更新为局部平均值
...
bool SensorTask_GetStats(SensorType_t type, SensorStats_t* stats) {
    if (!g_sensor_manager.is_initialized || type >= SENSOR_TYPE_MAX || stats == NULL) {
        return false;
    }
...
uint16_t SensorTask_GetPrimaryHistory(SensorType_t type, const float** history_data) {
    ...
    if (!g_sensor_manager.is_initialized || type >= SENSOR_TYPE_MAX || history_data == NULL) {
        *history_data = NULL;
        return 0;
    }
    ...
    if (sensor->is_enabled && sensor->history_count > 0) {
        ...
        if (osMutexWait(g_sensor_manager.global_data_mutex, 100) == osOK) {
            ...
            // 缓冲区未满：从索引0开始读取 / 缓冲区已满：从 history_head 的下一个位置开始（最旧的数据）
            int read_idx = (sensor->history_count < SENSOR_HISTORY_SIZE) ? 0 : sensor->history_head;
...
static void SensorTask_HandleSensorError(SensorInstance_t* sensor) {
    ...
    if (sensor->error_count == 10) {
        ...
        SensorTask_NotifyEvent(SENSOR_EVENT_STATUS_CHANGE, sensor->type, NULL, SENSOR_STATUS_ERROR);
    }
}
...
const char* SensorStatus_ToString(SensorStatus_t status) {
    switch (status) {
        ...
        case SENSOR_STATUS_INITIALIZING: return "INITIALIZING";
...
const char* SensorType_ToString(SensorType_t type) {
    switch (type) {
        ...
        case SENSOR_TYPE_GY30:      return "GY30";
        case SENSOR_TYPE_SHT30:     return "SHT30";
        case SENSOR_TYPE_SMOKE:     return "MQ-2";
        case SENSOR_TYPE_MAX:       return "MAX";
        default:                    return "UNKNOWN";
    }
}
...
const char* SensorEventType_ToString(SensorEventType_t event) {
    switch (event) {
        case SENSOR_EVENT_DATA_UPDATE:     return "DATA_UPDATE";
        case SENSOR_EVENT_STATUS_CHANGE:   return "STATUS_CHANGE";
        case SENSOR_EVENT_ERROR:           return "ERROR";
