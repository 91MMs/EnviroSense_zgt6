 ...
 * @file    lcd.h
 * @brief   LCD驱动头文件
 * @details 定义了LCD参数结构体、颜色定义及驱动函数声明。
 * @author  MmsY
 * @time    2025/11/23
 ******************************************************************************
 */
...
// #define LCD_WR_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOD_CLK_ENABLE();
// }while(0)   /* 所在IO口时钟使能 */

// #define LCD_RD_GPIO_PORT                GPIOD
// #define LCD_RD_GPIO_PIN                 GPIO_PIN_4
// #define LCD_RD_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOD_CLK_ENABLE();
// }while(0)   /* 所在IO口时钟使能 */
...
// // /* LCD_CS(需要根据LCD_FSMC_NEX设置正确的IO口) 和
// LCD_RS(需要根据LCD_FSMC_AX设置正确的IO口) 引脚 定义 */ #define
// LCD_CS_GPIO_PORT                GPIOG #define LCD_CS_GPIO_PIN GPIO_PIN_12
...
// #define LCD_RS_GPIO_CLK_ENABLE()        do{ __HAL_RCC_GPIOG_CLK_ENABLE();
// }while(0)   /* 所在IO口时钟使能 */
...
#define LCD_FSMC_BCRX                                                          \
  FSMC_Bank1                                                                   \
      ->BTCR[(LCD_FSMC_NEX - 1) * 2] /* BCR寄存器,根据LCD_FSMC_NEX自动计算 */
#define LCD_FSMC_BTRX                                                          \
  FSMC_Bank1->BTCR[(LCD_FSMC_NEX - 1) * 2 +                                    \
                   1] /* BTR寄存器,根据LCD_FSMC_NEX自动计算 */
#define LCD_FSMC_BWTRX                                                         \
  FSMC_Bank1E                                                                  \
      ->BWTR[(LCD_FSMC_NEX - 1) * 2] /* BWTR寄存器,根据LCD_FSMC_NEX自动计算 */
 ...
 *
 * 精英F103开发板使用FSMC_NE4连接LCD_CS, FSMC_A10连接LCD_RS
