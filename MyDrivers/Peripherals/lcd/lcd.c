...
uint32_t lcd_read_point(uint16_t x, uint16_t y) {
  ...
  return (
      ((r >> 11) << 11) | ((g >> 10) << 5) |
      (b >> 11)); /* ILI9341/NT35310/NT35510/ST7789/ILI9806需要公式转换一下 */
}
...
void lcd_display_dir(uint8_t dir) {
  ...
  if (dir == 0) /* 竖屏 */
  {
    ...
    if (lcddev.id == 0x5310 ||
        lcddev.id == 0x7796) /* 如果是5310/7796 则表示是 320*480分辨率 */
    {
      lcddev.width = 320;
      lcddev.height = 480;
    }
 ...
 * @retval      无
 */
// 注释部分已经在fsmc.c中实现了，这里注释避免重复定义
// 如果要使用触摸功能，需要修改touch.c的引脚
...
void lcd_init(void) {
  ...
  //    地址/数据线不复用 */ g_sram_handle.Init.MemoryDataWidth =
  //    FSMC_NORSRAM_MEM_BUS_WIDTH_16;    /* 16位数据宽度 */
  ...
  //    地址建立时间(ADDSET)为15个fsmc_ker_ck(1/168=6)即6*15=90ns */
  //    fsmc_read_handle.AddressHoldTime = 0x00;            /*
  //    地址保持时间(ADDHLD) 模式A是没有用到 */ fsmc_read_handle.DataSetupTime =
  //    60;                /* 数据保存时间(DATAST)为60个fsmc_ker_ck=6*60=360ns
  //    */
  //                                                        /*
  ...
  //    地址建立时间(ADDSET)为9个fsmc_ker_ck=6*9=54ns */
  //    fsmc_write_handle.AddressHoldTime = 0x00;           /*
  //    地址保持时间(ADDHLD) 模式A是没有用到 */ fsmc_write_handle.DataSetupTime
  //    = 9;                /* 数据保存时间(DATAST)为9个fsmc_ker_ck=6*9=54ns */
  //                                                        /*
    ...
    if (lcddev.id != 0x7789) /* 也不是ST7789, 尝试是不是 NT35310 */
    {
      ...
      if (lcddev.id != 0x5310) /* 也不是NT35310,尝试看看是不是ST7796 */
      {
        ...
        if (lcddev.id != 0x7796) /* 也不是ST7796,尝试看看是不是NT35510 */
        {
          ...
          delay_ms(5); /* 等待5ms, 因为0XC501指令对1963来说就是软件复位指令,
                          等待5ms让1963复位完成再操作 */
  ...
  /* 初始化完成以后,提速 */
  if (lcddev.id == 0x7789 || lcddev.id == 0x9341 ||
      lcddev.id == 0x1963) /* 7789/9341/1963 提速 */
  {
    // /* 重新配置写时序控制寄存器的时序 */
    // fsmc_write_handle.AddressSetupTime = 3; /*
    // 地址建立时间(ADDSET)为3个fsmc_ker_ck=6*3=18ns */
    // fsmc_write_handle.DataSetupTime = 3;    /*
    // 数据保持时间(DATAST)为3个fsmc_ker_ck=6*3=18ns */
    ...
    // [MODIFIED] 调用fsmc.c中提供的标准接口进行提速
    FSMC_Bank4_Write_Timing_Set(3, 3);
  } else if (lcddev.id == 0x5310 || lcddev.id == 0x7796 ||
             lcddev.id == 0x5510 ||
             lcddev.id == 0x9806) /* 如果是这几个IC, 则设置WR时序为最快 */
  {
    // /* 重新配置写时序控制寄存器的时序 */
    // fsmc_write_handle.AddressSetupTime = 2; /*
    // 地址建立时间(ADDSET)为2个fsmc_ker_ck=6*2=12ns */
    // fsmc_write_handle.DataSetupTime = 2;    /*
    // 数据保持时间(DATAST)为2个fsmc_ker_ck=6*2=12ns */
    // FSMC_NORSRAM_Extended_Timing_Init(g_sram_handle.Extended,
    // &fsmc_write_handle, g_sram_handle.Init.NSBank,
    // g_sram_handle.Init.ExtendedMode);

    // [MODIFIED] 调用fsmc.c中提供的标准接口进行提速
    FSMC_Bank4_Write_Timing_Set(5, 5);
  }
...
/**
 * @brief       在指定区域内填充单个颜色
 * @param       (sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex - sx + 1) * (ey
 * - sy + 1)
